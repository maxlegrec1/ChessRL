...
class FenEncoderMultiple(torch.nn.Module):
    def __init__(self, d_model,num_planes = 19):
        super().__init__()
        self.d_model = d_model//64
        self.num_planes = num_planes
        self.linear1 = torch.nn.Linear(num_planes,self.d_model)
        self.layernorm1 = torch.nn.LayerNorm(self.d_model)
        self.ma_gating = MaGating(self.d_model)
    
    def forward(self, x):
        x = torch.stack([torch.from_numpy(fen_to_tensor(fen)).to("cuda") for fen in x])
        x = x.view(-1,64,self.num_planes)
        x = self.linear1(x)
        x = torch.nn.GELU()(x)
        x = self.layernorm1(x)
        x = self.ma_gating(x)
        return x.view(-1,64*self.d_model)
