...
input_dir = "/media/maxime/385e67e0-8703-4723-a0bc-af3a292fd030/stockfish_data/Leela_training"
block_size = 64
clip_length = block_size  # Ensure moves are also clipped/padded to block_size
...
def batch_generator(input_dir, batch_size, return_fen=False, triple=False, device='cuda'):
    ...
    for path in files:
        ...
        for fen_initial, start_idx, moves_pgn_style in df.itertuples(index=False, name=None):
...
def _yield_batch(
    ...
    if return_fen_flag:
        ...
        if triple_flag:
            ...
            yield fens_batch_tensor, moves_batch_tensor, raw_fens_list_of_lists
        else:
            yield fens_batch_tensor, moves_batch_tensor


if __name__ == '__main__':
    from tqdm import tqdm
    # Example usage:
    # Adjust batch_size and number of iterations as needed for testing.
    # Ensure 'parquet_chunks' directory exists and has data.
    # Test with return_fen=True and triple=True as well.
    print(f"Using device: {'cuda' if torch.cuda.is_available() else 'cpu'}")
    # Test 1: Standard (FEN tensors, Move tensors)
    print("\nTesting standard batch generation:")
    try:
        gen_standard = batch_generator(input_dir, batch_size=4, device=('cuda' if torch.cuda.is_available() else 'cpu'))
        for i in range(2): # Print info for a few batches
            f_batch, m_batch = next(gen_standard)
            print(f"Batch {i+1}: FENs shape: {f_batch.shape}, Moves shape: {m_batch.shape}")
            print(f"FENs dtype: {f_batch.dtype}, Moves dtype: {m_batch.dtype}")
            # print(f"Example FEN tensor (first in batch):\n{f_batch[0]}")
            # print(f"Example Moves tensor (first in batch):\n{m_batch[0]}")
    except Exception as e:
        print(f"Error during standard test: {e}")

    # Test 2: Return FEN strings only
    print("\nTesting return_fen=True:")
    try:
        gen_fens_only = batch_generator(input_dir, batch_size=2, return_fen=True)
        for i in range(2):
            raw_fen_batch_data = next(gen_fens_only)
            print(f"Batch {i+1} (FEN strings only):")
            if isinstance(raw_fen_batch_data, list) and len(raw_fen_batch_data) > 0:
                print(f"  Number of games in batch: {len(raw_fen_batch_data)}")
                print(f"  Number of FENs in first game: {len(raw_fen_batch_data[0])}")
                # print(f"  First FEN string from first game: {raw_fen_batch_data[0][0]}")
                # print(f"  Last FEN string from first game: {raw_fen_batch_data[0][-1]}")
            else:
                print(f"  Received unexpected data format for raw FENs: {type(raw_fen_batch_data)}")
    except Exception as e:
        print(f"Error during return_fen test: {e}")

    # Test 3: Triple (FEN tensors, Move tensors, FEN strings)
    print("\nTesting triple=True:")
    try:
        gen_triple = batch_generator(input_dir, batch_size=32, triple=True, device=('cuda' if torch.cuda.is_available() else 'cpu'))
        for i in tqdm(range(50)):
            f_batch_t, m_batch_t, raw_fen_batch_t = next(gen_triple)
    ...
    # gen = batch_generator(input_dir, batch_size=32, device=('cuda' if torch.cuda.is_available() else 'cpu'))
