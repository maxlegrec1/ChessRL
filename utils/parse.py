...
def get_batch(pgn_path, return_fen = False, triple = False,batch_size = 16,all_elo = False):
    ...
    while True:
        ...
        if all_elo:
            elo = 3000
        if elo <= 1650 or 'FEN' in pgn.headers.keys() or '960' in pgn.headers['Event'] or 'Odds' in pgn.headers['Event'] or 'house' in pgn.headers['Event'] or 'Bullet' in pgn.headers['Event']:
            continue
