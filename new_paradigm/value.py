...
class BT4(torch.nn.Module):
    ...
    def forward(self,inp,compute_loss = False):
        ...
        if compute_loss:
            ...
            if true_values.shape[-1] == 1:
                loss_value = F.cross_entropy(value_h.view(-1,value_h.size(-1)), true_values.view(-1),ignore_index=3)
                return policy, value_h, loss_policy, loss_value, targets, true_values
            else: #true value shape is (batch_size, 3)
                loss_value = F.cross_entropy(value_h.view(-1,value_h.size(-1)), true_values.view(-1),ignore_index=3)
                #hard_targets = torch.argmax(true_values, dim=-1)
                #loss_value = F.cross_entropy(value_h.view(-1,value_h.size(-1)), hard_targets.view(-1))
                #value_h = torch.softmax(value_h, dim=-1)
                #print(value_h.shape,true_values.shape)
                #loss_value = F.mse_loss(value_h.view(-1,value_h.size(-1)), true_values.view(-1,3))
                return policy, value_h, loss_policy, loss_value, targets, torch.argmax(true_values, dim=-1)
        return policy
    ...
    def get_position_value(self, fen, device="cuda"):
        ...
        with torch.no_grad():
            ...
            value_logits = self.value_head(x_processed)
            value_logits = value_logits.view(b, seq_len, 3)
            
            # Apply softmax to get probabilities
            value_probs = F.softmax(value_logits, dim=-1)
            
        return value_probs.squeeze()  # Remove batch and sequence dimensions
    ...
    def get_batch_position_values(self, fens, device="cuda"):
        ...
        return value_probs.squeeze(1)  # Remove sequence dimension, keep batch dimension
    ...
    def calculate_move_values(self, fen, device="cuda"):
        ...
        batch_value_probs = self.get_batch_position_values(resulting_fens, device)
        ...
        if is_white_turn:
            ...
            player_values = 0.5 * batch_value_probs[:, 1] + batch_value_probs[:, 2]
        else:
            # Black's perspective: 1/2 * draw_prob + black_win_prob
            player_values = 0.5 * batch_value_probs[:, 1] + batch_value_probs[:, 0]
