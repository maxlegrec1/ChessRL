...
policy_index_leela_size = len(policy_index_leela)
policy_index_size = len(policy_index) 
# --- FUNCTION DEFINITIONS ---
def mirror_uci_string(uci_string):
    """
    Mirrors a uci string vertically (flips ranks 1-8).
    Assumes standard algebraic notation (files a-h, ranks 1-8).
    """
    # Check if it's a promotion move (e.g., e7e8q)
    if len(uci_string) == 5:
        ...
        # Flip ranks: rank 'y' becomes '9-y'
        return f"{start_file}{9 - int(start_rank)}{end_file}{9 - int(end_rank)}{promotion}"
    # Check if it's a standard move (e.g., e2e4)
        ...
        end_rank = uci_string[3]
        # Flip ranks: rank 'y' becomes '9-y'
        return f"{start_file}{9 - int(start_rank)}{end_file}{9 - int(end_rank)}"
    else:
        # Handle potential variations or errors, maybe return original or raise error
        # For now, returning original for unexpected formats
        print(f"Warning: Unexpected UCI string format found: {uci_string}")
        return uci_string

# --- MAPPING CREATION ---

# Create a dictionary for faster lookups of mirrored moves
policy_dict = {move: i for i, move in enumerate(policy_index)}

# mapping_black: Maps a Leela move index 'i' to the index 'j' where
# policy_index_leela[j] is the *mirrored* version of policy_index_leela[i].
# This is used to transform Black's policy (from White's perspective)
# into Black's actual policy (from Black's perspective).
mapping_black = np.zeros(policy_index_leela_size, dtype=np.int32)
for i in range(policy_index_leela_size):
    original_move = policy_index_leela[i]
    mirrored_move = mirror_uci_string(original_move)
    # Find the index of the mirrored move in the Leela list
    target_index = policy_dict.get(mirrored_move, -1) # Use dict for speed
    if target_index != -1:
        mapping_black[i] = target_index
    else:
        raise ValueError(f"move {mirrored_move} not found in policy_dict")

# mapping_white: Maps a Leela move index 'i' to the index 'j' where
# policy_index_leela[j] is the *same* move as policy_index_leela[i].
# This is essentially an identity mapping, as White's perspective doesn't change.
# The original code had a loop, but np.arange is sufficient and clearer.
mapping_white = np.arange(policy_index_leela_size, dtype=np.int32)

# --- MAPPING FUNCTION ---

def map_leela_to_rl(policies, first_is_white=True):
    # Ensure input is a numpy array for indexing
    policies = np.asarray(policies)
    batch_size = policies.shape[0]
    policy_len = policies.shape[1]

    if policy_len != len(policy_index_leela):
        raise ValueError(f"Input policies have length {policy_len}, but expected {len(policy_index_leela)} based on policy_index_leela.")

    # Initialize the result array
    res = np.zeros((batch_size,policy_index_size),dtype=np.float32)

    if first_is_white:
        white_indices = np.arange(0, batch_size, 2)
        black_indices = np.arange(1, batch_size, 2)
    else:
        white_indices = np.arange(1, batch_size, 2)
        black_indices = np.arange(0, batch_size, 2)

    # Apply identity mapping to White's policies (i.e., copy them directly)
    if len(white_indices) > 0:
        white_policies = policies[white_indices, :]
        res[np.ix_(white_indices, mapping_white)] = white_policies

    # Apply the mirror mapping to Black's policies
    if len(black_indices) > 0:
        black_policies =  policies[black_indices, :]
        # Fast assignment using advanced indexing if all indices are valid
        res[np.ix_(black_indices, mapping_black)] = black_policies
...
if __name__ == "__main__":
    import time
    test_policies = np.zeros((255, policy_index_leela_size))
    print("Original policies shape:", test_policies.shape)

    test_policies[0, 4] = 1
    test_policies[1,5] = 1
    print(policy_index_leela[4],policy_index[5]) #a1f1, a1g1
    #a1g1 should be converted to a8g8



    t0 = time.time()
    mapped_policies = map_leela_to_rl(test_policies, first_is_white=True)
    t1 = time.time()
    a1f1_index_rl = policy_index.index("a1f1")
    a8g8_index_rl = policy_index.index("a8g8")
    print("Mapped policies shape:", mapped_policies.shape)
    print(mapped_policies[0,a1f1_index_rl],mapped_policies[1,a8g8_index_rl])
    print(f"Time taken for mapping:{t1 - t0:.6f} seconds")