...
from new_paradigm.value import BT4

from utils.parse_value import batch_generator
...
dir_path = "../stockfish_data/4a100/"
gen = batch_generator(dir_path, batch_size=1600, block_size=1, return_fen=False, triple=True, device='cuda')
...
for i in progress_bar:
    ...
    with autocast(device_type='cuda', dtype=torch.float16, enabled= enable_float16):  # Enable mixed precision
        out,value,loss_policy, loss_value, targets, true_values = model(inp,compute_loss=True)
    ...
    if (i + 1) % gradient_accumulation_steps == 0:
        ...
        value_acc = (torch.argmax(value, dim=-1) == true_values).float()
        ...
        if step_counter % 5000 == 0:
            ...
            win_rate_vs_stockfish,elo = main_model_vs_stockfish(model=model,model1_name=f"{step_counter}", temp=0.1, num_games=40,elo = 2000)
            #win_rate_vs_model = main_model_vs_model(model1=model,model1_name=f"{step_counter}", temp=0.4, num_games=100)
        else:
            pass
        step_counter += 1
        progress_bar.set_description(f"Loss: {accumulated_loss:.4f} , Value Loss: {accumulated_value_loss:.4f} , Value Accuracy: {value_acc.item()}, Accuracy: {acc.item()}")
    ...
    if (i + 1) % 5000 == 0:
        #pass
        checkpoint_path = f"pretrain/v10_{i+1}.pt"
