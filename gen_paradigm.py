...
def gen(batch_size=32):
    ...
    for file_path in files:
        ...
        if isinstance(data, dict):
            game_initial_fens = data["fens"]
            game_variations_list = data["var"] # 'var' holds the variations
        elif isinstance(data, list): # Assuming list of dicts like [{'fen': ..., 'variations': ...}]
            game_initial_fens = [d.get('fen') for d in data]
            game_variations_list = [d.get('variations') for d in data]
        else:
            print(f"Warning: Unknown data format in {file_path}. Skipping.")
            continue
        for initial_fen, game_vars in zip(game_initial_fens, game_variations_list):
            print(len(game_vars))
